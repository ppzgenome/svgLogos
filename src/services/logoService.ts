...
export const searchLogoAlternative = async (term: string, currentUrl: string, currentProvider?: string): Promise<LogoSearchResult> => {
  ...
  // Start with providers we haven't used yet
  const unusedProviders = Object.entries(LOGO_SOURCES)
    .filter(([providerId]) => !termUsedSources.has(providerId))
    
  // Then add providers we've used before, but not the current one
  ...
  // Finally add the current provider as last resort
  const currentProviderEntry = currentProvider ? 
